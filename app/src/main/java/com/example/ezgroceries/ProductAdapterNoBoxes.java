...
public class ProductAdapterNoBoxes extends BaseAdapter {
    ArrayList<Object> list;
    public static final int PRODUCT = 0;
    public static final int HEADER = 1;
    private LayoutInflater inflater;

    public ProductAdapterNoBoxes(Context context, ArrayList<Object> list){
        this.list = list;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        if(convertView == null){
            switch (getItemViewType(position)){
                case PRODUCT:
                    convertView = inflater.inflate(R.layout.rowlayoutnoboxes, null);

                    break;
                case HEADER:
                    convertView = inflater.inflate(R.layout.headernoboxes,null);
                    break;
            }
        }
        switch (getItemViewType(position)){
